...
async function init() {
  ...
  const spNode = await setupScriptProcessor(recordBuffer, passSampleToVisualizers);
 ...
 */
async function setupScriptProcessor(recordBuffer, passSampleToVisualizers) {
  ...
  function onAudioProcess(event) {
    ...
    console.log(`event ${event.playbackTime}, curr ${context.currentTime}`);
  ...
  const processor = createWorkletScriptNode(context,
    onAudioProcess,
    256,
    {
      scopeVariables: {
